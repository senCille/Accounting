...
procedure TWOutputVAT.CrearFiltro;
begin
...
procedure TWOutputVAT.PrepararQuery;
begin
   QFichero.Close;
   QFichero.ModifySQL.Clear;
   QFichero.ModifySQL.Add('UPDATE IVAR SET TIPO = :TIPO, DESCRIPCION = :DESCRIPCION');
   QFichero.ModifySQL.Add('WHERE TIPO = :OLD_TIPO');

   QFichero.DisableControls;
   QFichero.Close;
   QFichero.SelectSQL.Clear;
   QFichero.SelectSQL.Add('SELECT TIPO, DESCRIPCION FROM IVAR');
...
procedure TWOutputVAT.RefrescarBD;
begin
   DMRef.QPaisNom.Close;
   DMRef.QPaisNom.Open;
end;

procedure TWOutputVAT.BtnAddClick(Sender: TObject);
begin
...
procedure TWOutputVAT.BtnDeleteClick(Sender: TObject);
begin
...
procedure TWOutputVAT.BtnReportClick(Sender: TObject);
begin
...
procedure TWOutputVAT.BtnSaveClick(Sender: TObject);
var
...
procedure TWOutputVAT.BtnCancelClick(Sender: TObject);
...
procedure TWOutputVAT.DataGridDblClick(Sender: TObject);
begin
...
procedure TWOutputVAT.FormCreate(Sender: TObject);
begin
...
procedure TWOutputVAT.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
   case Key of
      VK_ESCAPE: begin
...
procedure TWOutputVAT.RejillaTitleButtonClick(Sender: TObject; AFieldName: String);
begin
...
procedure TWOutputVAT.RejillaCalcTitleAttributes(Sender: TObject; AFieldName: String;
...
procedure TWOutputVAT.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TWOutputVAT.LimpiarFiltro(Sender: TObject);
begin
   if not (TbFiltro.state in dsEditModes) then begin
      TbFiltro.edit;
   end;
...
procedure TWOutputVAT.FormShow(Sender: TObject);
begin
   FormManager.Mode := fmEdit;
   FormManager.Mode := fmBrowse;
end;
